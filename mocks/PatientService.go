...
import (
	helpers "github.com/dr-ariawan-s-project/api-drariawan/utils/helpers"
...
// GetPagination provides a mock function with given fields: search, page, perPage
func (_m *PatientService) GetPagination(search string, page int, perPage int) (helpers.Pagination, error) {
	ret := _m.Called(search, page, perPage)

	var r0 helpers.Pagination
	var r1 error
	if rf, ok := ret.Get(0).(func(string, int, int) (helpers.Pagination, error)); ok {
		return rf(search, page, perPage)
	}
	if rf, ok := ret.Get(0).(func(string, int, int) helpers.Pagination); ok {
		r0 = rf(search, page, perPage)
	} else {
